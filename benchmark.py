...
def run_once(problem_text: str, gold_text: str, console_lock: bool, lemma_pool: list):
    ...
    _console_log('[benchmark-worker] begin', print_enabled=has_print_permission)

    try:
        if has_print_permission:
            _console_log(f"[benchmark-worker] print_to_console={has_print_permission} ...", print_enabled=has_print_permission)
    ...
    if has_print_permission:
        _console_log('[benchmark-worker] end, evaluate', print_enabled=has_print_permission)
    ...
    if has_print_permission:
        _console_log('[benchmark-worker] end', print_enabled=has_print_permission)
...
def main():
    ...
    print(f"[benchmark] starting {args.runs} runs ...", flush=True)

    # 多进程并行：避免多线程共享同一进程带来的 GIL/CPU 抢占与 stdout 交错。
    max_worker_num = max(1, (os.cpu_count() or 1) - 2)
    futures = [ ]

    print(f"[benchmark] starting {max_worker_num} workers for parallel ...", flush=True)
    ...
    print(f"[benchmark] detailed results saved to: {out_path}", flush=True)
