...
AlphaSolve 通过 `install.bat` 脚本一键安装，自动处理 Python 和所有依赖，不需要单独安装 Python。

1. 打开 https://github.com/tanzcoding/AlphaSolve ，点击绿色的 **Code** 按钮，选择 **Download ZIP**，将压缩包下载到本地
2. 将下载的 ZIP 文件**解压**到任意一个文件夹（例如桌面上的 `AlphaSolve` 文件夹）
3. 在解压后的文件夹中，**双击 `install.bat`**（Windows）或在终端中运行 `sh install.sh`（macOS / Linux）

安装完成后，`alphasolve` 命令全局可用。
...
你会看到一个实时面板，显示 AlphaSolve 正在工作的各个阶段。不要关闭终端，让它运行即可。如果想中途停止，直接关闭窗口即可。
