...
class RoleWorkspaceAccess:
    ...
    def _resolve_writable_file(self, path: str) -> Path:
        ...
        write_root = self.workspace.resolve(self.write_root_rel)
        if target != write_root and write_root not in target.parents:
            raise ValueError(f"write path must stay under {self.write_root_rel}")
        return target
...
class SubagentService:
    ...
    def call(self, agent_type: str, task: str, *, depth: int = 0) -> dict[str, Any]:
        ...
        registry = self._build_subagent_registry(depth=depth)
    ...
    def _build_subagent_registry(self, *, depth: int) -> ToolRegistry:
        registry = ToolRegistry()
        ...
        registry.register(
            ...
            handler=lambda args: _python_tool(args, python_env),
        )
        registry.register(
            name="run_wolfram",
            ...
            handler=lambda args: _wolfram_tool(args, wolfram_session),
        )
...
def _python_tool(args: dict[str, Any], env: dict[str, Any]) -> ToolResult:
    stdout, error = run_python(str(args.get("code") or ""), env=env)
    payload = {}
    if stdout:
...
def _wolfram_tool(args: dict[str, Any], session_ref: dict[str, Any]) -> ToolResult:
    try:
        if session_ref.get("session") is None:
            from wolframclient.evaluation import WolframLanguageSession

            session_ref["session"] = WolframLanguageSession()
        output, error = run_wolfram(str(args.get("code") or ""), session=session_ref["session"])
    except Exception as exc:
        return ToolResult(json.dumps({"error": str(exc)}, ensure_ascii=False), is_error=True)
    payload = {}
    if output:
