...
def _portfolio_checkpoint_prompt(artifact_path: Path | None, *, recovery_reason: str = "") -> str:
    ...
    return (
        ...
        + recovery
        + "Read the current DAG and curation_input.json before recording facts. Reconcile runtime handoff IDs into canonical IDs only when cited evidence establishes the same mathematical obligation; otherwise archive the obstacle as an attempt without creating a node. Existing aliases in difficulty_dag.json are binding: when a handoff is already mapped, reuse that canonical difficulty rather than assigning a new one. Add a child only when cited mathematics establishes a separately checkable, strictly smaller dependency; parentless components are valid. When a verified proposition directly contradicts an existing node's statement, submit a cited status_updates entry with status=refuted; an obstacle report alone never changes status. Reviewer observations are evidence pointers only: verify cited propositions, audits, or handoffs before changing a canonical node, edge, or status. Preserve a parent edge only when cited evidence establishes the mathematical relationship. Record attempt evidence and canonical mappings; never choose the next route, rank nodes, infer a pivot from repeated unchanged attempts, create a strategy rule, or maintain tabu policy. Write evidence-bounded knowledge notes only."
    )
