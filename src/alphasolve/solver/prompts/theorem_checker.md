...
- Read cited files in `verified_propositions` when the proposition uses `\ref{path-without-extension}`. The path is relative to `verified_propositions`, omits `.md`, and uses Windows backslashes for subdirectories, such as `\ref{number-theory\order-lifting}`.
